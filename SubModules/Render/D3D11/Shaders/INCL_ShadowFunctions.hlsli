...
float PCF_Filter(float2 uv, float zReceiverNDC, float filterRadiusUV, Texture2D shadowMap, SamplerComparisonState shadowCompareSampler)
{
	float sum = 0.0f;
	for (int i = 0; i < 64; ++i)
	{
...
void FindBlocker(out float avgBlockerDepthView, out float numBlockers, float2 uv, float zReceiverView, Texture2D shadowMap, SamplerState shadowPointSampler, matrix invProj, float lightRadiusWorld, float nearZ)
{
...
// honglab course
float PCSS(float2 uv, float zReceiverNDC, Texture2D shadowMap, SamplerState shadowPointSampler, SamplerComparisonState shadowCompareSampler, matrix invProj, float lightRadiusWorld, float nearZ)
{
	float lightRadiusUV = lightRadiusWorld / LIGHT_FRUSTRUM_WIDTH;
	...
	FindBlocker(avgBlockerDepthView, numBlockers, uv, zReceiverView, shadowMap, shadowPointSampler, invProj, lightRadiusWorld, nearZ);
	...
	else
	{
        ...
        // STEP 3: filtering
		return PCF_Filter(uv, zReceiverNDC, filterRadiusUV, shadowMap, shadowCompareSampler);
	}	
}

float GetShadowFactor(ShadowInput input)
{
	// current pixel's pos into light's space
	float4 positionLightScreen = mul(float4(input.posWorld, 1), input.light.view);
	positionLightScreen = mul(positionLightScreen, input.light.proj);
	...
	return PCSS(texcoordLightSpace, positionLightScreen.z - BIAS, input.shadowMap, input.shadowPointSampler, input.shadowCompareSampler, input.light.invProj, input.light.radius, input.nearZ);
}
#endif /* __SHADOW_FUNCTIONS__ */