...
void D3D11DeviceResources::CreateDeviceResources()
{
...
void D3D11DeviceResources::CreateWindowSizeDependentResources()
{
	if (!m_window)
	{
...
void D3D11DeviceResources::SetWindow(HWND window, int width, int height) noexcept
{
	m_window = window;
...
bool D3D11DeviceResources::WindowSizeChanged(int width, int height)
{
	if (!m_window)
		return false;
...
void D3D11DeviceResources::HandleDeviceLost()
{
	if (m_deviceNotify)
	{
...
void D3D11DeviceResources::Present()
{
	HRESULT hr = E_FAIL;
	if (m_options & c_AllowTearing)
	{
...
void D3D11DeviceResources::CreateFactory()
{
...
void D3D11DeviceResources::GetHardwareAdapter(IDXGIAdapter1** ppAdapter)
{
	*ppAdapter = nullptr;
...
void D3D11DeviceResources::UpdateColorSpace()
{
	if (!m_dxgiFactory)
		return;
